...
def _app_today(user) -> date:
    """The service's notion of 'today': the user's app day (05:00-local
    rollover — Tier 1.2). Streak-walking tests must anchor to the same
    boundary the service uses or they flake around UTC midnight."""
...
def test_log_advances_streak_to_streak_toast(authed_client, db):
    ...
    streak.last_logged_date = _app_today(authed_client.test_user) - timedelta(days=1)
...
def test_log_advances_to_milestone_toast(authed_client, db):
    ...
    streak = StreakTracking(
        ...
        last_logged_date=_app_today(authed_client.test_user) - timedelta(days=2),
    )
    db.add(streak)
    db.commit()
